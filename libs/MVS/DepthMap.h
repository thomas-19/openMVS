...
#define ComposeDepthFilePath(i, e) \
  MAKE_PATH(String::FormatString(("depth%04u." + String(e)).c_str(), i))
...
struct MVS_API DepthData {
  struct ViewData {
    ...
    inline void Init(const Camera& cameraRef) {
      ...
      if (!depthMap.empty()) {
        ...
        Tm = cameraDepthMap.K * cameraDepthMap.R *
             (cameraRef.C - cameraDepthMap.C);
        Tr = cameraRef.K * cameraRef.R * cameraDepthMap.R.t() *
             cameraDepthMap.GetInvK();
        Tn = cameraRef.K * cameraRef.R * (cameraDepthMap.C - cameraRef.C);
      }
    }

    inline IIndex GetID() const { return pImageData->ID; }
    ...
    static bool ScaleImage(const IMAGE& image, IMAGE& imageScaled,
                           float scale) {
      if (!NeedScaleImage(scale)) return false;
      cv::resize(image, imageScaled, cv::Size(), scale, scale,
  ...
  ViewDataArr images;      // array of images used to compute this depth-map
                           // (reference image is the first)
  ViewScoreArr neighbors;  // array of all images seeing this depth-map (ordered
  ...
  ViewsMap viewsMap;      // view-IDs map (indexing images vector starting after
                          // first view)
  float dMin, dMax;       // global depth range for this image
  unsigned references;  // how many times this depth-map is referenced (on 0 can
                        // be safely unloaded)
  ...
  inline bool IsValid() const { return !images.IsEmpty(); }
  inline bool IsEmpty() const { return depthMap.empty(); }

  const ViewData& GetView() const { return images.front(); }
  const Camera& GetCamera() const { return GetView().camera; }

  void GetNormal(const ImageRef& ir, Point3f& N,
                 const TImage<Point3f>* pPointMap = NULL) const;
  void GetNormal(const Point2f& x, Point3f& N,
                 const TImage<Point3f>* pPointMap = NULL) const;
...
struct MVS_API DepthEstimator {
  ...
  enum {
    nTexels =
        SQUARE((nSizeHalfWindow * 2 + nSizeStep) / nSizeStep) * TexelChannels
  };

  enum ENDIRECTION { LT2RB = 0, RB2LT, DIRS };
...
#if DENSE_SMOOTHNESS == DENSE_SMOOTHNESS_NA
  CLISTDEF0IDX(NeighborData, IIndex)
  neighbors;  // neighbor pixels coordinates to be processed
#else
  CLISTDEF0IDX(ImageRef, IIndex)
  neighbors;  // neighbor pixels coordinates to be processed
#endif
#if DENSE_SMOOTHNESS != DENSE_SMOOTHNESS_NA
  CLISTDEF0IDX(NeighborEstimate, IIndex)
  neighborsClose;  // close neighbor pixel depths to be used for smoothing
#endif
  Point3 X0;      //
...
#if DENSE_NCC != DENSE_NCC_WEIGHTED
  const Image64F&
      image0Sum;  // integral image used to fast compute patch mean intensity
#endif
  ...
  DepthEstimator(unsigned nIter, DepthData& _depthData0,
  ...
  bool FillPixelPatch();
  float ScorePixelImage(const DepthData::ViewData& image1, Depth,
                        const Normal&);
  ...
  inline float GetImage0Sum(const ImageRef& p) const {
    ...
    return (float)(image0Sum(p3) - image0Sum(p2) - image0Sum(p1) +
                   image0Sum(p0));
  }
#endif
  ...
  inline Matrix3x3f ComputeHomographyMatrix(const DepthData::ViewData& img,
                                            Depth depth,
  ...
  static inline Point3 ComputeRelativeC(const DepthData& depthData) {
    return depthData.images[1].camera.R *
           (depthData.images[0].camera.C - depthData.images[1].camera.C);
  }
  static inline Matrix3x3 ComputeRelativeR(const DepthData& depthData) {
    RMatrix R;
    ComputeRelativeRotation(depthData.images[0].camera.R,
                            depthData.images[1].camera.R, R);
    return R;
  }
  ...
  inline Normal RandomNormal(const Point3f& viewRay) {
    Normal normal;
    Dir2Normal(Point2f(rnd.randomRange(FD2R(0.f), FD2R(180.f)),
  ...
  inline void CorrectNormal(Normal& normal) const {
    ...
    if (cosAngLen >= 0)
      normal = RMatrixBaseF(
                   normal.cross(viewDir),
                   MINF((ACOS(cosAngLen / norm(viewDir)) - FD2R(90.f)) * 1.01f,
                        -0.001f)) *
               normal;
    ASSERT(ISEQUAL(norm(normal), 1.f));
  }

  static bool ImportIgnoreMask(const Image&, const Image8U::Size&,
                               uint16_t nIgnoreMaskLabel, BitMatrix&,
                               Image8U* = NULL);
  static void MapMatrix2ZigzagIdx(const Image8U::Size& size,
                                  DepthEstimator::MapRefArr& coords,
                                  const BitMatrix& mask, int rawStride = 16);
...
// Tools
bool TriangulatePoints2DepthMap(const DepthData::ViewData& image,
                                const PointCloud& pointcloud,
                                const IndexArr& points, DepthMap& depthMap,
                                NormalMap& normalMap, Depth& dMin, Depth& dMax,
                                bool bAddCorners, bool bSparseOnly = false);
bool TriangulatePoints2DepthMap(const DepthData::ViewData& image,
                                const PointCloud& pointcloud,
                                const IndexArr& points, DepthMap& depthMap,
                                Depth& dMin, Depth& dMax, bool bAddCorners,
                                bool bSparseOnly = false);

// Robustly estimate the plane that fits best the given points
MVS_API unsigned EstimatePlane(const Point3Arr&, Plane&, double& maxThreshold,
                               bool arrInliers[] = NULL, size_t maxIters = 0);
MVS_API unsigned EstimatePlaneLockFirstPoint(const Point3Arr&, Plane&,
                                             double& maxThreshold,
                                             bool arrInliers[] = NULL,
                                             size_t maxIters = 0);
MVS_API unsigned EstimatePlaneTh(const Point3Arr&, Plane&, double maxThreshold,
                                 bool arrInliers[] = NULL, size_t maxIters = 0);
MVS_API unsigned EstimatePlaneThLockFirstPoint(const Point3Arr&, Plane&,
                                               double maxThreshold,
                                               bool arrInliers[] = NULL,
                                               size_t maxIters = 0);
MATH_API int OptimizePlane(Planed& plane, const Eigen::Vector3d* points,
                           size_t size, int maxIters, double threshold);
// same for float points
MATH_API unsigned EstimatePlane(const Point3fArr&, Planef&,
                                double& maxThreshold, bool arrInliers[] = NULL,
                                size_t maxIters = 0);
MATH_API unsigned EstimatePlaneLockFirstPoint(const Point3fArr&, Planef&,
                                              double& maxThreshold,
                                              bool arrInliers[] = NULL,
                                              size_t maxIters = 0);
MATH_API unsigned EstimatePlaneTh(const Point3fArr&, Planef&,
                                  double maxThreshold, bool arrInliers[] = NULL,
                                  size_t maxIters = 0);
MATH_API unsigned EstimatePlaneThLockFirstPoint(const Point3fArr&, Planef&,
                                                double maxThreshold,
                                                bool arrInliers[] = NULL,
                                                size_t maxIters = 0);
MATH_API int OptimizePlane(Planef& plane, const Eigen::Vector3f* points,
                           size_t size, int maxIters, float threshold);

MVS_API void EstimatePointColors(const ImageArr& images,
                                 PointCloud& pointcloud);
MVS_API void EstimatePointNormals(
    const ImageArr& images, PointCloud& pointcloud,
    int numNeighbors = 16 /*K-nearest neighbors*/);

MVS_API bool EstimateNormalMap(const Matrix3x3f& K, const DepthMap&,
                               NormalMap&);
...
MVS_API bool SaveConfidenceMap(const String& fileName,
                               const ConfidenceMap& confMap);
MVS_API bool LoadConfidenceMap(const String& fileName, ConfidenceMap& confMap);

MVS_API Image8U3 DepthMap2Image(const DepthMap& depthMap,
                                Depth minDepth = FLT_MAX, Depth maxDepth = 0);
MVS_API bool ExportDepthMap(const String& fileName, const DepthMap& depthMap,
                            Depth minDepth = FLT_MAX, Depth maxDepth = 0);
MVS_API bool ExportNormalMap(const String& fileName,
                             const NormalMap& normalMap);
MVS_API bool ExportConfidenceMap(const String& fileName,
                                 const ConfidenceMap& confMap);
MVS_API bool ExportPointCloud(const String& fileName, const Image&,
                              const DepthMap&, const NormalMap&);
...
MVS_API bool ExportDepthDataRaw(const String&, const String& imageFileName,
                                ...
                                Depth dMin, Depth dMax, const DepthMap&,
                                const NormalMap&, const ConfidenceMap&,
                                const ViewsMap&);
MVS_API bool ImportDepthDataRaw(const String&, String& imageFileName,
                                IIndexArr&, cv::Size& imageSize, KMatrix&,
                                RMatrix&, CMatrix&, Depth& dMin, Depth& dMax,
                                DepthMap&, NormalMap&, ConfidenceMap&,
                                ViewsMap&, unsigned flags = 15);

MVS_API bool GetDepthMapHeaderSize(const String& fileName, cv::Size& size);

MVS_API bool ResizeDepthDataRaw(const String& fileName, const cv::Size& size);

MVS_API void CompareDepthMaps(const DepthMap& depthMap,
                              const DepthMap& depthMapGT, uint32_t idxImage,
                              float threshold = 0.01f);
MVS_API void CompareNormalMaps(const NormalMap& normalMap,
